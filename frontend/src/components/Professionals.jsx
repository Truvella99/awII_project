...
function Professionals({loggedIn, role, unreadMessages, pending, setPending, setUnreadMessages}) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if (filterModel.employmentState) {
                if (filterModel.employmentState.type === 'contains') {
                    if ( !((item.employmentState.toLowerCase()).includes(filterModel.employmentState.filter.toLowerCase())) )
                        continue;
    ...
    const [columnDefs, setColumnDefs] = useState([
        ...
        { field: "dailyRate", filter: "agNumberColumnFilter", filterParams: {filterOptions: ["equals", "lessThan", "greaterThan", "inRange"]}, headerName: "Daily Rate", suppressHeaderMenuButton: true, valueFormatter: dailyRateFormatter },
